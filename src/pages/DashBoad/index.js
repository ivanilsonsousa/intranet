    ...
    <>
      ...
      <div className="container">
        <div className="container-fluid d-flex align-items-baseline w-100">
          ...
          <button type="button" className="btn btn-light ml-auto" onClick={() => setModal(true)} ><strong>SAIR</strong> <i className="fas fa-sign-out-alt"></i></button>
        </div>
        <hr className="my"></hr>
        <div className="container content-dashboard" style={{ backgroundImage: `url(${logo})` }}>
          <Option image={group_icon} legend="Usuários" to="/dashboard/users" backgroundColor="#3C52B4" />
          ...
          <Option image={video_icon} legend="Vídeos" to="/dashboard/videos" backgroundColor="#A3D98C" />
        </div>
      </div>
      <Modal title={"Deseja realmente sair do sistema"} show={modal} onDisable={setModal} func={() => handleLogout()} />
    </>
  )
}
